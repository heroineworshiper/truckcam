# compare webcam vijeo with a given face image
# python3 face_detect2.py --image2=face3.jpg --face_detection_model=yunet.onnx --face_recognition_model=face_recognizer_fast.onnx
...
#    print("1 face_detection_model=%s score_threshold=%f nms_threshold=%f top_k=%f" % (args.face_detection_model, \
#        args.score_threshold, \
#        args.nms_threshold, \
#        args.top_k))
    ## [initialize_FaceDetectorYN]
# 4 on the odroid is optimum.  It defaults to 8.
    cv.setNumThreads(4)
...
# get the face features from the ref image
        if args.image2 is not None:
            img2 = cv.imread(cv.samples.findFile(args.image2))
            detector.setInputSize((img2.shape[1], img2.shape[0]))
            faces2 = detector.detect(img2)
# create the recognizer
        ...
        while cv.waitKey(1) < 0:
            ...
            # compare faces
            if faces2[1] is not None and \
                faces[1] is not None:
                face1_align = recognizer.alignCrop(frame, faces[1][0])
                face1_feature = recognizer.feature(face1_align)
                cosine_score = recognizer.match(face1_feature, face2_feature, cv.FaceRecognizerSF_FR_COSINE)
                print("cosine score=%f" % cosine_score)
